...
#[utoipa::path(
    tag = "Addresses",
    get,
    path = "/addresses/{address}/charms/utxos/{charm}",
    params(
    ...
    responses(
        (status = 200, description = "Charm UTXOs for specific charm at address", body = ServeResponse<Vec<CharmUtxo>>),
        (status = 400, description = "Malformed query parameters"),
        (status = 404, description = "Not found"),
        (status = 500, description = "Internal server error"),
    )
)]
/// Charm UTXOs by Address and Charm
///
/// Returns UTXOs controlled by the provided address which contain the specified charm, sorted by height.
pub async fn addresses_specific_charm_utxos(
    State(state): State<AppState>,
    Query(params): Query<MempoolParam>,
    Path((address, charm)): Path<(String, String)>,
    ...
    for kv in iter {
        ...
        // Skip UTXOs that don't contain the specified charm
        if !charms.iter().any(|c| c.app == canonical_app) {
            continue;
        }
